...
## 2026-07-15 · 修复：「显示隐藏文件」意图被裸文件名别名拦截

### 背景
玩家输入「显示隐藏文件 录音」时，预期按文件夹批量揭示录音目录下的隐藏文件，但实际却打开了某个录音文件内容。问题出在 `detect_intent()` 的优先级顺序上。

### 问题诊断
`hybrid_reply.py` 中 `_extract_filename()` 为 `audio/` 子目录下的录音文件统一注册了别名「录音」。在 `detect_intent()` 中，裸文件名匹配（read 意图）位于 `show_hidden` 意图检测之前，因此输入「显示隐藏文件 录音」时：
1. 先命中「录音」→ 被识别为 read 意图；
2. 绕过了后方的 `show_hidden` 检测。

### 修复方案
将 `install_skill` 和 `show_hidden` 的显式意图检测提前到裸文件名匹配之前，确保「显示隐藏文件」「安装隐藏文件技能」等明确操作指令优先于文件别名解析。

### 改动文件
- `engine/hybrid_reply.py` — `detect_intent()` 中调整意图检测顺序
- `GDD/07-tech.md` — 补充 `install_skill` / `show_hidden` 意图说明及优先级约束
- `GDD/08-iteration-log.md`（本文件）
- `CHANGELOG.md`

### 验证结果
- ✅ 输入「显示隐藏文件 录音」→ 触发 `show_hidden`，按文件夹批量揭示隐藏文件
- ✅ 直接输入「录音」→ 仍保持 read 意图（打开录音文件）
- ✅ 其他操作意图（如「安装隐藏文件技能」）未被文件名别名误拦截
