...
**方案：**
- 在 `engine/hybrid_reply.py` 的 `clue` 意图分支中，根据 `target_id` 检查该文件夹是否已全部解锁。
- 已解锁的「待扫描文件夹」条目临时归类为「文件线索」后再渲染。
- 新增 `_is_target_unlocked()` 辅助函数，供线索分类和密码提示复用。

**修改文件：**
- `engine/hybrid_reply.py` — `_is_target_unlocked()` + clue 意图预处理
- `GDD/07-tech.md` — 补充线索分类与已解锁迁移说明
- `GDD/08-iteration-log.md` — 追加记录
- `CHANGELOG.md` — 本条目
- `knowledge/triggers/passwords.json` — VPN 解锁提示文案微调

---

## 2026-07-15 · 修复：「显示隐藏文件」意图被文件名别名误拦截


**背景：**
- 输入「显示隐藏文件 录音」时，预期按文件夹批量揭示隐藏文件，实际却打开了一个录音文件内容。
- 根因：`engine/hybrid_reply.py` 中 `_extract_filename()` 为录音文件注册了「录音」别名，且裸文件名匹配（read 意图）在 `show_hidden` 意图检测之前执行。

**方案：**
- 将 `install_skill` 和 `show_hidden` 的显式意图检测提前到裸文件名匹配之前，确保明确操作指令优先于文件别名解析。
- 同步更新 GDD 技术文档中的意图列表和调用图。

**修改文件：**
- `engine/hybrid_reply.py` — `detect_intent()` 意图顺序调整
- `GDD/07-tech.md` — 补充 `install_skill` / `show_hidden` 意图及优先级约束
- `GDD/08-iteration-log.md` — 追加修复记录
- `CHANGELOG.md` — 本条目
...
**背景：**
- 侧边栏点击文件会额外弹窗，打断聊天流
- 「文件」按钮只展开侧边栏，没有告诉玩家当前文件状态
- AI 建议栏文本与按钮分散在两端，视觉上不紧凑
- 工作日记全部读完后仍推荐继续读，不会推进到下一目标
- 问「你是谁」时偶发回答成张知予
- 玩家输入空「获取」时不知道当前有哪些未解锁文件夹

**方案：**
- 移除文件查看弹窗，点击文件只在聊天区显示文件卡片
- 「文件」按钮改为请求「查看文件状态」，后端返回三类文件清单并自动展开侧边栏
- AI 建议栏改为 `justify-content: flex-start`，按钮与文本间距统一为 12px
- 前后端建议生成同步：按顺序指向下一篇未读工作日记，全部读完后推进到私人文件夹密码
- 响应库 `who_am_i_curious` 去掉错误变体，回答统一为 M-M 自我介绍
- 空目标/未识别目标「获取」时列出已发现但未解锁的文件夹，无发现则引导阅读文档

**修改文件：**
- `index.html` — 移除文件弹窗 DOM/CSS/JS；文件按钮改为请求状态；建议栏左对齐；前端建议逻辑同步
- `engine/hybrid_reply.py` — `files` 意图返回文件状态；`_build_default_suggestions` 修复工作日记推进；新增 `_build_get_folder_hint`；`handle_get_command` 空目标提示
- `knowledge/response-library.json` — 修复「你是谁」回答变体

---

## 2026-07-13 · 重构框架：目标发现、主线引导与密码格式统一
...
**方案：**
- 目标发现：读取含线索的文件后自动发现对应文件夹，未发现的目标不能扫描/获取
- 主线建议：根据章节、已读文件、已发现目标动态生成下一步建议
- UI 重构：建议按钮合并到 AI 建议栏右侧，文本保持左对齐
- 执行建议：点击按钮直接把建议作为输入发送
- 密码脱敏：前端展示「获取 工作日记 密码」，不泄露明文
- 密码格式统一：支持 `获取 文件夹名 密码` 和 `获取 文件夹名 具体密码`

**新增文件：**
- `engine/folder_discovery.py` — 文件夹发现管理
- `knowledge/folder-discoveries.json` — 文件→目标发现配置表

**修改文件：**
- `engine/hybrid_reply.py` — 目标发现集成、扫描/获取发现检查、密码格式解析、建议生成
- `engine/qa_engine.py` — 增加 `folder_help` 优先级
- `knowledge/qa-library.json` — 新增 `folder_help` 帮助条目
- `index.html` — 建议栏 UI 重构、执行建议按钮、密码脱敏
- `GDD/07-tech.md` / `GDD/08-iteration-log.md`
